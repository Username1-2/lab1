...
std::unordered_map<Trit, int, std::hash<int>> Tritset::Cardinality() {
    ...
    for(size_t i = 0; i < Length(); i++) res[(*this)[i]]++;
    return res;
}
...
/*Tritset::Reference& Tritset::Reference::operator&=(const Reference& x) {
    *this &= x.trit;
    return *this;
}

Trit Tritset::Reference::operator&(Trit t) const {
    Trit res = Unknown;
    if(trit == False) res = False;
    if(trit == Unknown) {
        if(t != False) res = Unknown;
        else res = False;
    }
...
Trit Tritset::Reference::operator&(const Tritset::Reference &x) const {
    return *this & x.trit;
}*/
