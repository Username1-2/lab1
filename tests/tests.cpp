...
TEST(TritsetTest, Tritset_Cardinality) {
    ...
    ASSERT_EQ(cardMap[False], 0);
    ASSERT_EQ(cardMap[Unknown], 0);
    ASSERT_EQ(cardMap[True], 0);
    size_t count = set.Capacity()/3, i;
    for(i = 0; i + 2 < set.Capacity(); i += 3) {
        set[i] = False;
        set[i+1] = Unknown;
        set[i+2] = True;
    }
    ASSERT_EQ(set.Cardinality(False), count);
    ASSERT_EQ(set.Cardinality(Unknown), count);
    ASSERT_EQ(set.Cardinality(True), count);
    cardMap = set.Cardinality();
    ASSERT_EQ(cardMap[False], count);
    ASSERT_EQ(cardMap[Unknown], count);
    ASSERT_EQ(cardMap[True], count);
}
